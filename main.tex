...
\author{
    \IEEEauthorblockN{Gautam Singh, Sachin Omprakash Dubey and G.V.V. Sharma} 
    %\\
...
usually given by the RSSI. The RSSI (in
...
RSSI measurements in place but at various orientations. The UGV reads the RSSI values at 3 locations 90\degree apart.  The decision phase
...
\section{Results}
\label{sec:results}
In the AARD algorithm, the UGV eventually converges close to the beacon (here, the hotspot). However, in some cases, when the RSSI values are too close, the UGV may not be able to track the beacon and may end up in a random direction.  The LARD algorithm is more reliable.  However,  in this case, 
...
extension to our proposed algorithm could be to combine the AARD and LARD algorithms to reduce the time and total number
of steps taken by adjusting the value of \(N, \theta\) as the UGV converges to the
